...
export default function WarehouseGuard({ isCafe, children }) {
  ...
  const checkAndSetWarehouse = async () => {
    ...
    try {
      ...
      const targetWarehouse = warehouses.data.find((w) => w.isCafe === isCafe);

      if (!targetWarehouse) {
        throw new Error(
          isCafe ? "Kafe ombori topilmadi" : "Asosiy ombor topilmadi",
        );
      }
  ...
  // Loading holati
  // FIX: avval bu yerda {children} ham darhol ko'rsatilardi — ya'ni
  // cafeWarehouseId/mainWarehouseId hali API'dan kelmasdan turib
  // foydalanuvchi sahifa bilan ishlashni boshlab yuborardi (masalan,
  // "To'lov qilish" tugmasini bosib ulgurardi). Shu sabab modal ochilganda
  // ba'zida cafeWarehouseId hali null bo'lib qolardi. Endi ID tayyor
  // bo'lguncha faqat yuklanish chizig'i ko'rsatiladi, sahifa kontenti esa
  // ID kelgandan keyingina render qilinadi.
  if (isLoading) {
