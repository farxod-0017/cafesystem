...
export default function CashsPage() {
  ...
  const fetchPayMethods = async () => {
    try {
      ...
      const filtered = res.data.filter(
        (pm) => pm.locationId === selectedLocation?.id,
      );
      setItems(filtered);
    } finally {
  ...
  const handleFillCash = async () => {
    ...
    try {
      setSubmitting(true);
      await apiPaymentCash.Add({
        amount: parseFloat(pul),
        cashId: fillingCash.id,
        type: "deposit",
    ...
    <Box p={6}>
      ...
      {pageLoading ? (
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
          {items.map((item) => (
            <Card key={item.id} bg={"surface"} rounded="xl" role="group">
              <CardBody>
      ...
      <Modal isOpen={isFillOpen} onClose={onFillClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Kassani to'ldirish</ModalHeader>
          <ModalBody>
            <FormLabel>
              {fillingCash?.name} - kassani to'ldirish uchun miqdor
            </FormLabel>
            <Input
              mb={2}
              type="number"
      ...
      <Modal isOpen={isTakeOpen} onClose={onTakeClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Kassadan pul yechish</ModalHeader>
          <ModalBody>
            <FormLabel>
              {takingCash?.name} - kassadan yechish uchun miqdor
            </FormLabel>
            <Input
              mb={2}
              type="number"
